...
void icylib_lazy_chunked_draw_rounded_rectangle(icylib_LazyChunkedImage* image, int x1, int y1, int x2, int y2, int radius, icylib_Color color, unsigned char blend);

void icylib_lazy_chunked_fill_rounded_rectangle(icylib_LazyChunkedImage* image, int x1, int y1, int x2, int y2, int radius, icylib_Color color, unsigned char blend);
...
void icylib_lazy_chunked_draw_circular_arc(icylib_LazyChunkedImage* image, int x, int y, int radius, float phi1, float phi2, icylib_Color color, unsigned char blend);

void icylib_lazy_chunked_fill_circular_arc(icylib_LazyChunkedImage* image, int x, int y, int radius, float phi1, float phi2, icylib_Color color, unsigned char blend);
...
void icylib_lazy_chunked_draw_rounded_rectangle(icylib_LazyChunkedImage* image, int x1, int y1, int x2, int y2, int radius, icylib_Color color, unsigned char blend) {
    icylib_lazy_chunked_draw_line(image, x1 + radius, y1, x2 - radius, y1, color, blend, 0);
    icylib_lazy_chunked_draw_line(image, x1 + radius, y2, x2 - radius, y2, color, blend, 0);
    icylib_lazy_chunked_draw_line(image, x1, y1 + radius, x1, y2 - radius, color, blend, 0);
    icylib_lazy_chunked_draw_line(image, x2, y1 + radius, x2, y2 - radius, color, blend, 0);
    icylib_lazy_chunked_draw_circular_arc(image, x1 + radius, y1 + radius, radius, 1.0 * ICYLIB_PI, 1.5 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_draw_circular_arc(image, x1 + radius, y2 - radius, radius, 0.5 * ICYLIB_PI, 1.0 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_draw_circular_arc(image, x2 - radius, y1 + radius, radius, 1.5 * ICYLIB_PI, 2.0 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_draw_circular_arc(image, x2 - radius, y2 - radius, radius, 0.0 * ICYLIB_PI, 0.5 * ICYLIB_PI, color, blend);
}

void icylib_lazy_chunked_fill_rounded_rectangle(icylib_LazyChunkedImage* image, int x1, int y1, int x2, int y2, int radius, icylib_Color color, unsigned char blend) {
    icylib_lazy_chunked_fill_rectangle(image, x1 + radius, y1 + radius, x2 - radius, y2 - radius, color, blend);
    icylib_lazy_chunked_fill_rectangle(image, x1 + radius, y1, x2 - radius, y1 + radius, color, blend);
    icylib_lazy_chunked_fill_rectangle(image, x1 + radius, y2 - radius, x2 - radius, y2, color, blend);
    icylib_lazy_chunked_fill_rectangle(image, x1, y1 + radius, x1 + radius, y2 - radius, color, blend);
    icylib_lazy_chunked_fill_rectangle(image, x2 - radius, y1 + radius, x2, y2 - radius, color, blend);
    icylib_lazy_chunked_fill_circular_arc(image, x1 + radius, y1 + radius, radius, 1.0 * ICYLIB_PI, 1.5 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_fill_circular_arc(image, x1 + radius, y2 - radius, radius, 0.5 * ICYLIB_PI, 1.0 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_fill_circular_arc(image, x2 - radius, y1 + radius, radius, 1.5 * ICYLIB_PI, 2.0 * ICYLIB_PI, color, blend);
    icylib_lazy_chunked_fill_circular_arc(image, x2 - radius, y2 - radius, radius, 0.0 * ICYLIB_PI, 0.5 * ICYLIB_PI, color, blend);
}
...
void icylib_lazy_chunked_draw_circular_arc(icylib_LazyChunkedImage* image, int x, int y, int radius, float phi1, float phi2, icylib_Color color, unsigned char blend) {
    ...
    icylib_draw_bresenham_circular_arc((unsigned char*)image, x, y, radius, phi1, phi2, color, (void (*)(unsigned char*, int, int, icylib_Color))(blend ? icylib_lazy_chunked_set_pixel_blend : icylib_lazy_chunked_set_pixel));
}
...
void icylib_lazy_chunked_draw_text(icylib_LazyChunkedImage* image, char* text, int x, int y, icylib_Color color, icylib_Font font, int pixel_size, icylib_HorizontalAlignment horizontal_alignment, icylib_VerticalAlignment vertical_alignment, icylib_HorizontalTextFitting horizontal_fitting, icylib_VerticalTextFitting vertical_fitting, unsigned char blend) {
    icylib_draw_text((unsigned char*)image, text, x, y, color, font, pixel_size, horizontal_alignment, vertical_alignment, horizontal_fitting, vertical_fitting, (void (*)(unsigned char*, int, int, icylib_Color))(blend ? icylib_lazy_chunked_set_pixel_blend : icylib_lazy_chunked_set_pixel));
}
