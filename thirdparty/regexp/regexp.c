#ifndef REGEXP_MALLOC
#define REGEXP_MALLOC malloc
#endif
#ifndef REGEXP_FREE
#define REGEXP_FREE free
#endif
...
Reprog *regcomp(const char *pattern, int cflags, const char **errorp)
{
	...
	if (setjmp(g.kaboom)) {
		...
		REGEXP_FREE(g.pstart);
		REGEXP_FREE(g.prog);
		return NULL;
	}

	g.prog = REGEXP_MALLOC(sizeof (Reprog));
	if (!g.prog)
		die("cannot allocate regular expression");
	n = strlen(pattern) * 2;
	if (n > 0) {
		g.pstart = g.pend = REGEXP_MALLOC(sizeof (Renode) * n);
		if (!g.pstart)
	...
	g.prog->nsub = g.nsub;
	g.prog->start = g.prog->end = REGEXP_MALLOC(n * sizeof (Reinst));
...
void regREGEXP_FREE(Reprog *prog)
{
	if (prog) {
		REGEXP_FREE(prog->start);
		REGEXP_FREE(prog);
	}
}
